...
class NumDeMed:Salidas{
    private:
        float NumeroMedicion;
    public:
        NumDeMed(float,float,float,float);
};
Salidas::Salidas(float _x,float _y,float _z){ //constructor
    x = _x;
    y = _y;
    z = _z;
    xG = (x/.65*(3/1.65)-3);
...
NumDeMed::NumDeMed(float _x,float _y,float _z, float M):Salidas(_x,_y,_z){//herencia
    NumeroMedicion = M;
}


void Salidas::imprimir(){
...
int main(){
    Salidas S(1.65,0.0,3.3);
    NumDeMed N(1.65,0.0,3.3,2.3);
    return(0);
}